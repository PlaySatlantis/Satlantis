...
local function emphasisParse(text, state)
    ...
    while finished == false do
        if text:find("<%S.-%S>", last) then --url
            ...
            --local url = text:sub(start+1,finish-1)
            --text = text:sub(1,start-1) .. "<action name=link url="..url.."><style color=#77AAFF>" .. url .. "</style></action>" .. text:sub(finish+1)
            ...
            if string.match(text, "https://") or string.match(text, "http://") then
                local url = text:gsub("<style color=#77AAFF>", ""):gsub("</style>", ""):gsub(" ", "")
                text = "<action name=md_url|"..url.. " url="..url..">"..text.."</action>"
            end
            finished = true
        end
    end
...
minetest.register_on_player_receive_fields(function (player, formname, fields)
    if not fields.markdown then return end

    local url = string.split(fields.markdown, "md_url|")[2]

    if url then
        local prefix = minetest.colorize("#dff6f5", "[LINK] ")
        local arrow = minetest.colorize("#dff6f5", " ====> ")
        minetest.chat_send_player(player:get_player_name(), "\n\n"..prefix ..  "If the smartphone link doesn't work, click on this with your mouse wheel (make sure clickable chat links are enabled in your Minetest settings)\n".. arrow..url .. " \n")
    end
end)